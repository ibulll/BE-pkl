    ...
    public function siswaDibimbing(Request $request)
{
    try {
        ...
        // Ambil data pengajuan PKL yang memiliki pembimbing_id yang sesuai
        $pengajuanPkl = PengajuanPKL::where('pembimbing_id', $pembimbingId)->get();

        // Kumpulkan id user siswa dari pengajuan PKL yang dibimbing oleh pembimbing
        $siswaIds = $pengajuanPkl->pluck('user_id')->toArray();

        // Ambil data siswa berdasarkan id yang terkumpul, hanya ambil id, nisn, nama, dan email
