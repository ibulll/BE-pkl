...
class PdfController extends Controller
{
    ...
    public function verifyPdf(Request $request)
    {
        ...
        $request->validate([
            ...
            'nama' => 'required|string', // Nama pengguna yang ingin diverifikasi
        ]);
    
        try {
            // Terima nomor surat dan nama pengguna dari request
            $nomorSurat = $request->input('nomor_surat');
            $name = $request->input('nama');
    
            // Misalnya, Anda memiliki pola nama file yang sesuai dengan nama pengguna
            $pdfPaths = [];
    
            // Membuat nama file PDF berdasarkan nama pengguna
            $pdfName = "pengajuan_sppd_$name.pdf";
            $pdfPath = public_path("storage/pdfs/$pdfName");
            ...
            return response()->json(['message' => 'PDF paths retrieved successfully', 'nomor_surat' => $nomorSurat, 'pdf_paths' => $pdfPaths]);
        } catch (\Exception $e) {
            // Jika terjadi kesalahan, tangani kesalahan dan kirim respons kesalahan ke frontend
            return response()->json(['error' => 'Failed to retrieve PDF paths: ' . $e->getMessage()], 500);
        }
    }
}
